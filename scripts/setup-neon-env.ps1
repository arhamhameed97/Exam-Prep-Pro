# Neon Environment Setup Script for Vercel Deployment (PowerShell)
# This script helps you set up environment variables for Vercel deployment with Neon

Write-Host "======================================" -ForegroundColor Cyan
Write-Host "  Exam Prep App - Neon + Vercel Setup" -ForegroundColor Cyan
Write-Host "======================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "Enter your Neon DATABASE_URL (pooled connection recommended):" -ForegroundColor Yellow
Write-Host "(Format: postgresql://user:pass@host/database?sslmode=require)" -ForegroundColor Gray
...
Write-Host ""
Write-Host "Enter your Neon DIRECT_URL (for migrations, optional):" -ForegroundColor Yellow
$DIRECT_URL = Read-Host "DIRECT_URL (or press Enter to skip)"
...
Write-Host "DIRECT_URL: $($DIRECT_URL.Substring(0, [Math]::Min(30, $DIRECT_URL.Length)))..."
...
function Add-EnvVar {
    ...
    # Use vercel env add command with echo
...
if (-not [string]::IsNullOrWhiteSpace($DIRECT_URL)) {
    Add-EnvVar -Name "DIRECT_URL" -Value $DIRECT_URL -Environment "production"
}
...
if (-not [string]::IsNullOrWhiteSpace($DIRECT_URL)) {
    Add-EnvVar -Name "DIRECT_URL" -Value $DIRECT_URL -Environment "preview"
}
...
if ($ADD_DEV -eq "y" -or $ADD_DEV -eq "Y") {
    ...
    if (-not [string]::IsNullOrWhiteSpace($DIRECT_URL)) {
        Add-EnvVar -Name "DIRECT_URL" -Value $DIRECT_URL -Environment "development"
    }
fi
...
Write-Host "1. Deploy your app: vercel --prod"
Write-Host "2. Run migrations: npx prisma db push"
Write-Host "3. Check deployment logs for any errors"
Write-Host "4. Test authentication on your deployed app"
Write-Host ""
